<!-- File: /features/admin/content-security/components/ScanRunModal.vue -->
...
const subtitle = computed(() => {
  switch (scanMode.value) {
    case 'specific':
      return 'Configure scan parameters for a specific content object';
    case 'content_type':
      return 'Configure scan parameters for an entire content type';
    case 'everything':
      return 'Configure system-wide scan across all content types';
    default:
      return '';
  }
});
...
watch([debouncedScanObjectQuery, scanMode], ([_, newMode]) => {
  if (newMode === 'specific') {
