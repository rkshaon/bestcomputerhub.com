...
<template>
  <UiCard class="flex items-center gap-6 p-8">
    <div :class="cn('w-14 h-14 rounded-2xl flex items-center justify-center shrink-0', color || 'bg-indigo-100 text-indigo-600')">
      <component :is="icon" class="w-7 h-7" />
    </div>
    <div class="flex-1 min-w-0">
      <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 mb-1">{{ label }}</p>
