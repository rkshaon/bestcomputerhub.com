...
Always enforce demand-driven API calling: never dispatch API requests on component mount, watcher triggers, or route initialization unless the data is immediately required by the current view. Reuse available state/props/store data before issuing new requests, and fetch auxiliary workflow data (such as option lists or detailed entity records) only when the specific user workflow demands it. For paginated filter option endpoints across Admin and Storefront, implement infinite scroll (`useInfinitePagination` / `<UiInfiniteScroll />`) to load and append option pages on scroll until `next` is `null`, avoiding duplicate in-flight requests, preserving loaded options across filter reopenings, and debouncing option search queries (300ms).
