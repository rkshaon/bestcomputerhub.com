...
import CategoryViewModal from '@/features/admin/categories/components/CategoryViewModal.vue';
...
<template>
  <NuxtLayout name="admin">
          ...
          <div class="mt-5 pt-3 border-t border-border/60 flex items-center justify-between">
            ...
            <div class="flex items-center gap-1">
              ...
              <button 
                ...
                @click="openFeaturedIconModal(cat)" 
                class="p-2 text-muted-foreground hover:text-primary hover:bg-muted rounded-lg transition-all cursor-pointer relative"
                title="Manage Featured Icon"
                aria-label="Manage featured icon"
              >
                <ImageIcon class="w-4 h-4" />
                <span v-if="cat.featured_icon" class="absolute top-1 right-1 w-2 h-2 rounded-full bg-primary ring-2 ring-card"></span>
              </button>
              <button 
                v-if="canEditCategory"
    ...
    <CategoryTreeAdmin
      ...
      @featured-icon="openFeaturedIconModal"
    />
      ...
      <template #cell-actions="{ item: cat }">
        <div class="flex items-center justify-end gap-1 opacity-90 group-hover:opacity-100 transition-opacity">
          ...
          <button 
            v-if="canEditCategory"
            @click="openFeaturedIconModal(cat)" 
            class="p-1.5 text-muted-foreground hover:text-primary hover:bg-muted rounded-md transition-all cursor-pointer relative"
            title="Manage Featured Icon"
            aria-label="Manage featured icon"
          >
            <ImageIcon class="w-3.5 h-3.5" />
            <span v-if="cat.featured_icon" class="absolute -top-0.5 -right-0.5 w-1.5 h-1.5 rounded-full bg-primary ring-2 ring-card"></span>
          </button>
          <button 
            v-if="canEditCategory"
            @click="triggerEditModal(cat)" 
    ...
    <!-- Read-Only Category Inspection Modal -->
    <CategoryViewModal
      :is-open="isViewModalOpen && !!selectedCategory"
      :category="selectedCategory"
      :is-resolving="isDetailsLoading || categoryModalState.isResolving.value"
      :categories="allCategoriesList"
