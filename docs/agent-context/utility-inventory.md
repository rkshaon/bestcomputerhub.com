...
| :--- | :--- | :--- | :--- |
| `useProductService` | Products & Images | `/api/v1/products/`, `/api/v1/product-images/`, `/api/v1/product-images/summary/` | Product list queries, brand ID filtering (`brands`), price mapping (`current_selling_price`), primary image fallback, product image summary statistics (`getProductImageSummary`), boolean filters (`is_high_resolution`, `is_ratio_mismatch`), trailing slashes, bulk product image uploads. |
| `useCategoryService` | Categories & Tree | `/api/v1/categories/`, `/api/v1/categories/tree/`, `/api/v1/categories/{id-or-slug}/price-range/` | Category hierarchy tree traversal, sibling-level accordion expansion (`expandedCategoryIds`), demand-driven lazy child batching, breadcrumb path construction (`getCategoryPath`), category price range fetching (`getCategoryPriceRange`), trailing slashes. |
| `useBrandService` | Brands | `/api/v1/brands/`, `/api/v1/brands/category/{category_id}/` | Brand listing, active status filtering, brand CRUD operations, category-specific brand listing (`getBrandsByCategory`), brand response mapping. |
| `useBlogService` | Blog Posts & Tags | `/api/v1/blog/posts/`, `/api/v1/blog/tags/`, `/api/v1/blog/posts/{id}/categories/`, `/api/v1/blog/posts/{id}/categories/{cat_id}/` | Blog post queries by tag/category/author, post detail by ID/slug, blog post category assignment (`assignBlogPostCategories`), blog post category removal (`removeBlogPostCategory`), blog tag CRUD operations. |
