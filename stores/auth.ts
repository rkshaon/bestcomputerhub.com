...
import { useCookie, navigateTo, callWithNuxt, useNuxtApp } from '#app';
    ...
    async logout(redirectPath?: string) {
      ...
      return callWithNuxt(nuxtApp, async () => {
        ...
        try {
          ...
          const target = redirectPath || (currentPath && currentPath !== '/login' && currentPath !== '/signup' ? currentFullPath : '');
          if (target) {
